...
function getEnv() {
  return {
    ...
    siteUrl: process.env.VITE_SITE_URL || `https://${DOMAIN}`,
  }
}

/** Create or link a local user from a Forumline identity (exported for direct use) */
export async function createOrLinkUser(identity: ForumlineIdentity, hubAccessToken: string | null): Promise<string> {
...
/** Generate a Supabase session for a user after auth (exported for direct use) */
export async function afterAuth({ userId }: { userId: string }): Promise<string | undefined> {
  const { supabaseUrl, supabaseAnonKey, serviceRoleKey, siteUrl } = getEnv()
  const supabase = createClient(supabaseUrl, serviceRoleKey)
