...
export function createMobileTabBar({ forumStore, onChangeView }: MobileTabBarOptions): MobileTabBarInstance {
  ...
  function render() {
    ...
    forumsBtn.className = `tab-bar__item${currentView === 'forums' ? ' tab-bar__item--active' : ''}`
    forumsBtn.innerHTML = `
    ...
    dmsBtn.className = `tab-bar__item${currentView === 'dms' ? ' tab-bar__item--active' : ''}`
    const dmBadge = dmUnread > 0
      ? `<div class="badge badge--red badge--inline" style="position:absolute;right:-8px;top:-4px;min-width:16px;height:16px;font-size:9px">${dmUnread > 99 ? '99+' : dmUnread}</div>`
      : ''
    dmsBtn.innerHTML = `
      <div class="relative">
      ...
      <span class="tab-bar__label">DMs</span>
    `
    ...
    settingsBtn.className = `tab-bar__item${currentView === 'settings' ? ' tab-bar__item--active' : ''}`
